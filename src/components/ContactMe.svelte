<section class="mt-40 max-w-lg mx-auto" id="section-4"> 
    <div class="mx-auto">
      <p class="font-bold md:text-left text-sm text-primary-600">CONTACT</p>
      <p class="font-extrabold md:text-left text-md text-black my-3 dark:text-white">Don't hesitate! Hit me up!👇🏻</p>
    </div>
    <div class="flex flex-row md:flex-col justify-center">
      <div class="grid md:grid-cols-2">
        <div class="p-6 max-w-sm mx-auto bg-white rounded-xl flex items-center space-x-5 dark:bg-slate-600">
          <div class="">
            <svg class="w-6 h-6 text-primary-700" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 16 20">
              <path d="M8 0a7.992 7.992 0 0 0-6.583 12.535 1 1 0 0 0 .12.183l.12.146c.112.145.227.285.326.4l5.245 6.374a1 1 0 0 0 1.545-.003l5.092-6.205c.206-.222.4-.455.578-.7l.127-.155a.934.934 0 0 0 .122-.192A8.001 8.001 0 0 0 8 0Zm0 11a3 3 0 1 1 0-6 3 3 0 0 1 0 6Z"/>
            </svg>
          </div>
          <div class="text-left space-y-1">
            <p class="text-sm font-bold text-black dark:text-white">Location</p>
            <p class="text-xs text-slate-500">Seongnam, South Korea</p>
          </div>
        </div>
        <div class="p-6 bg-white rounded-xl flex items-center space-x-5 dark:bg-slate-600">
          <div class="">
            <svg class="w-6 h-6 text-primary-700" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
              <path d="m9.978 13.233 9.392-6.668a1.945 1.945 0 0 0-.186-.177L11.2.65A2 2 0 0 0 8.815.638L.8 6.4a1.928 1.928 0 0 0-.207.2l9.385 6.633Z"/>
              <path d="M11.181 14.864a2.007 2.007 0 0 1-2.382-.014L0 8.627V18a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V8.573l-8.819 6.29Z"/>
            </svg>
          </div> 
          <div class="text-left space-y-1">
            <p class="text-sm font-bold text-black dark:text-white">Mail</p>
            <p class="text-xs text-slate-500">danjang0123@gmail.com</p>
          </div>
        </div>
      </div>

    </div>
</section>

